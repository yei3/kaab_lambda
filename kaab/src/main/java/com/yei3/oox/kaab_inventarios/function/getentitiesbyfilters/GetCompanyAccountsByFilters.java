...
public class GetCompanyAccountsByFilters implements RequestStreamHandler {

    @Override
    public void handleRequest(InputStream input, OutputStream output, Context context) throws IOException {
    	LambdaLogger logger = context.getLogger();
        logger.log("Getcompanyaccountsbyfilters starting");
        	...
        	if (event.get("multiValueQueryStringParameters") != null) {
                ...
                if ( pps.get("column") != null && pps.get("condition") != null && pps.get("value") != null) {
                	...
                	if (columns.size() == conditions.size() && columns.size() == values.size()) {
                    	...
                    	List<Object> companys = (List<Object>)h.getListByConditions(CompanyAccount.class, columnsString, conditionsString, valuesString);
                    	CompanyAccount companyAccount;
                    	JSONObject companyJson;
                    	
            			for (int i = 0; i < companys.size(); i++) {
            				companyAccount = (CompanyAccount)companys.get(i); 
