...
public class GetMenusByFilters implements RequestStreamHandler {

    @Override
    public void handleRequest(InputStream input, OutputStream output, Context context) throws IOException {
    	LambdaLogger logger = context.getLogger();
        logger.log("Getmenusbyfilters starting");
        	...
        	if (event.get("multiValueQueryStringParameters") != null) {
                ...
                if ( pps.get("column") != null && pps.get("condition") != null && pps.get("value") != null) {
                	...
                	if (columns.size() == conditions.size() && columns.size() == values.size()) {
                    	...
                    	List<Object> menus = (List<Object>)h.getListByConditions(Menu.class, columnsString, conditionsString, valuesString);
                    	Menu menu;
                    	JSONObject menuJson;
            			...
            			for (int i = 0; i < menus.size(); i++) {
            				...
            				menuJson.put("menu", menu.getMenu());
            				menuJson.put("description", menu.getDescription());
            				menuJson.put("parentID", menu.getParentID());
