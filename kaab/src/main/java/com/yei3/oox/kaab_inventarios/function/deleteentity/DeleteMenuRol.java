...
public class DeleteMenuRol implements RequestStreamHandler {

    @Override
    public void handleRequest(InputStream input, OutputStream output, Context context) throws IOException {
    	LambdaLogger logger = context.getLogger();
        logger.log("DeletemenuRol starting");
        ...
        try {
        	...
        	MenuRol menuRol = (MenuRol)h.getItemById(MenuRol.class, toIntExact((long)body.get("id")));
        	
        	if (menuRol != null) {
        		menuRol.setStatusID(1);
            	//TODO add cognito >:v
        		User user = (User)h.getItemById(User.class, toIntExact((long)body.get("userId")));
            	if (user != null) {
            		menuRol.setDeleteUserID(toIntExact((long)body.get("userId")));
            		menuRol.setDeleteDateTime(new Timestamp(System.currentTimeMillis()));
                	
                	h.updateItem(MenuRol.class, menuRol);
