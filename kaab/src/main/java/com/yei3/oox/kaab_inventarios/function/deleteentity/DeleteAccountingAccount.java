...
public class DeleteAccountingAccount implements RequestStreamHandler {

    @Override
    public void handleRequest(InputStream input, OutputStream output, Context context) throws IOException {
    	LambdaLogger logger = context.getLogger();
        logger.log("DeleteaccountingAccount starting");
        ...
        try {
        	...
        	if (accountingAccount != null) {
            	accountingAccount.setStatusID(1);
            	//TODO add cognito >:v
            	User user = (User)h.getItemById(User.class, toIntExact((long)body.get("userId")));
            	if (user != null) {
            		accountingAccount.setDeleteUserID(toIntExact((long)body.get("userId")));
                	accountingAccount.setDeleteDateTime(new Timestamp(System.currentTimeMillis()));
