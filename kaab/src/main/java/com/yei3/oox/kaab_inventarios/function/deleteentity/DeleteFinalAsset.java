...
public class DeleteFinalAsset implements RequestStreamHandler {

    @Override
    public void handleRequest(InputStream input, OutputStream output, Context context) throws IOException {
    	LambdaLogger logger = context.getLogger();
        logger.log("DeletefinalAsset starting");
        ...
        try {
        	...
        	if (finalAsset != null) {
            	finalAsset.setStatusID(1);
            	//TODO add cognito >:v
            	User user = (User)h.getItemById(User.class, toIntExact((long)body.get("userId")));
            	if (user != null) {
            		finalAsset.setDeleteUserID(toIntExact((long)body.get("userId")));
                	finalAsset.setDeleteDateTime(new Timestamp(System.currentTimeMillis()));
