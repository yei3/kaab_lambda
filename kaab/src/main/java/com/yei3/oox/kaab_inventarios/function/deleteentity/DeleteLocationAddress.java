...
public class DeleteLocationAddress implements RequestStreamHandler {

    @Override
    public void handleRequest(InputStream input, OutputStream output, Context context) throws IOException {
    	LambdaLogger logger = context.getLogger();
        logger.log("DeletelocationAddress starting");
        ...
        try {
        	...
        	LocationAddress locationAddress = (LocationAddress)h.getItemById(LocationAddress.class, toIntExact((long)body.get("id")));
        	
        	if (locationAddress != null) {
            	locationAddress.setStatusID(1);
            	//TODO add cognito >:v
            	User user = (User)h.getItemById(User.class, toIntExact((long)body.get("userId")));
            	if (user != null) {
            		locationAddress.setDeleteUserID(toIntExact((long)body.get("userId")));
                	locationAddress.setDeleteDateTime(new Timestamp(System.currentTimeMillis()));
                	
                	h.updateItem(LocationAddress.class, locationAddress);
